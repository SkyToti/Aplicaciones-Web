/* ============================================================
   Repaso API REST — NÚCLEO
   Lo que comparten todas las páginas: utilidades, guardado, tema,
   iconos, sonido, cabecera y pie, avance y la hoja imprimible.
   ============================================================ */
...
// página: no se cambia, o tus compañeros pierden su avance.
const store = {
...
/* ============================================================
   ...
   El orden de esta lista ES la ruta de estudio: de aquí salen el menú,
   los botones «Anterior / Siguiente» y las tarjetas del inicio.
   ============================================================ */
const PAGINAS = [
  { id: 'inicio', archivo: 'index.html', nombre: 'Inicio', corto: 'Inicio', icono: 'house', tono: 'am' },
  {
    id: 'fundamentos', archivo: 'fundamentos.html', nombre: 'Fundamentos', corto: 'Fundamentos', icono: 'book-open', tono: 'az', min: 20,
    desc: 'Qué es una API, REST en 6 reglas, las partes de un endpoint, los métodos HTTP y la arquitectura por capas.'
  },
  {
    id: 'codigos', archivo: 'codigos.html', nombre: 'Códigos HTTP', corto: 'Códigos', icono: 'traffic-cone', tono: 've', min: 15,
    desc: 'Las 5 familias, los 18 códigos y cuándo se usa cada uno, los duelos que confunden y el juego de parejas.'
  },
  {
    id: 'laboratorio', archivo: 'laboratorio.html', nombre: 'Laboratorio', corto: 'Laboratorio', icono: 'flask-conical', tono: 'mo', min: 15,
    desc: 'Simulador de peticiones, constructor de endpoints y validador de rutas. Aquí se practica razonando.'
  },
  {
    id: 'ejercicio', archivo: 'ejercicio.html', nombre: 'El ejercicio', corto: 'Ejercicio', icono: 'pencil-ruler', tono: 'am', min: 25,
    desc: 'Reglas de nombrado, los 6 giros resueltos, arma tu propia API y el checklist de la entrega.'
  },
  {
    id: 'examen', archivo: 'examen.html', nombre: 'Examen', corto: 'Examen', icono: 'timer', tono: 'ro', min: 15,
...
function avancePagina(id) {
  ...
  switch (id) {
    case 'inicio': return store.get('cheat', []).length / CHEAT.length;
    ...
    case 'ejercicio': return (store.get('chk', []).length / CHECK.length) * .5 + (si('armarOk') ? .5 : 0);
    case 'examen': return Math.min(1, (store.get('best', 0) / QUIZ.length) * .4 + (store.get('bestExamen', 0) / 10) * .6);
  }
  return 0;
}
...
function progreso() {
  const pct = Math.round(PAGINAS.reduce((t, p) => t + avancePagina(p.id), 0) / PAGINAS.length * 100);
  const f = $('#progFill'), t = $('#progPct');
  if (f) f.style.width = pct + '%';
  if (t) t.textContent = pct + '%';
  $$('.tab[data-p]').forEach(a => a.classList.toggle('lista', avancePagina(a.dataset.p) >= .99));
  ...
  <header class="bar">
    <div class="bar-in">
      <a class="logo" href="index.html" aria-label="Inicio">
        ...
        <span class="logo-txt">Repaso <b>REST</b></span>
      </a>
      <nav class="tabs" aria-label="Páginas">
        ${PAGINAS.map(p => `<a href="${p.archivo}" class="tab${p.id === pag ? ' on' : ''}" data-p="${p.id}"${p.id === pag ? ' aria-current="page"' : ''}>${p.corto}</a>`).join('')}
      </nav>
      <div class="bar-right">
        <div class="prog" title="Tu avance total"><div class="prog-fill" id="progFill"></div><span id="progPct">0%</span></div>
...
function inyectarPie(pag) {
  ...
  const i = PAGINAS.findIndex(p => p.id === pag);
  const ant = i > 0 ? PAGINAS[i - 1] : null, sig = PAGINAS[i + 1] || null;
  c.outerHTML = `
  <footer class="foot">
    ...
    <p class="counter">Hecho por Diego para el grupo de Aplicaciones Web.</p>
    <div class="row-mid">
      <button class="btn btn-ghost btn-s" data-imprimir><i data-lucide="printer"></i> Imprimir la chuleta</button>
      <button class="btn btn-ghost btn-s" id="wipe">Borrar mi avance</button>
    </div>
  </footer>
...
/* Quien compartió el link viejo (una sola página con #quiz, #giros…)
   aterriza en el inicio: se le manda a la sección nueva que corresponde. */
const ANCLAS_VIEJAS = {
...
function redirigirAnclasViejas(pag) {
  if (pag !== 'inicio') return false;
  const destino = ANCLAS_VIEJAS[location.hash.slice(1)];
  if (destino) { location.replace(destino); return true; }
  return false;
}

/* Borra el avance, pero no las preferencias (tema y sonido). */
const LLAVES_AVANCE = ['cheat', 'chk', 'best', 'bestExamen', 'examenes', 'fallos', 'giro', 'mBest', 'parejas',
  'orden', 'armar', 'armarOk', 'usoSim', 'usoConst', 'ultima',
  'visto_fundamentos', 'visto_codigos', 'visto_laboratorio', 'visto_ejercicio', 'visto_examen'];
...
document.addEventListener('DOMContentLoaded', () => {
  const pag = document.body.dataset.pagina || 'inicio';
  ...
  if (pag !== 'inicio') store.set('ultima', pag);
  ...
  if (wipe) wipe.addEventListener('click', () => {
    if (!confirm('¿Borrar todo tu avance? Se pierden tus fallos, tus récords y tu progreso. No se puede deshacer.')) return;
    LLAVES_AVANCE.forEach(k => store.del(k));
    location.reload();
  });
